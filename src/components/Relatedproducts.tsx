...
const RelatedProducts: React.FC = () => {
  ...
  const goTo = (id: string) => navigate(`/product/${id}`);
  const showMore = () => navigate(`/shop/${current.category}`);
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto py-14 px-5 flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6 xl:gap-8 w-full max-w-[1236px] mb-8">
        {related.map((product) => {
          ...
          const isNew = (() => {
            ...
            const diff =
              (new Date().getTime() - pd.getTime()) / (1000 * 60 * 60 * 24);
            return diff <= 30;
          })();
